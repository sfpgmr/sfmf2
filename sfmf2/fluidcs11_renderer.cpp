...
//--------------------------------------------------------------------------------------
template <class T>
HRESULT CreateConstantBuffer(ID3D11Device* d3d_device, ID3D11Buffer** ppCB)
{
  HRESULT hr = S_OK;
...
HRESULT CreateStructuredBuffer(Device& d3d_device, UINT iNumElements, ID3D11BufferPtr& ppBuffer, ID3D11ShaderResourceViewPtr& ppSRV, ID3D11UnorderedAccessViewPtr& ppUAV, const T* pInitialData = NULL)
{
  HRESULT hr = S_OK;
...
//--------------------------------------------------------------------------------------
void create_shader_blob_from_file(const std::wstring& path, const std::string& entry_point, const std::string& shader_model, ID3DBlobPtr& blob_out, const D3D_SHADER_MACRO* defines = nullptr)
{
  HRESULT hr = S_OK;
...
void create_shader(DevicePtr& device, const void * blob_ptr, SIZE_T s, ID3D11ClassLinkage * l, ID3D11VertexShaderPtr& shader_ptr)
{
  CHK(device->CreateVertexShader(blob_ptr, s, l, &shader_ptr));
};

template <typename DevicePtr>
void create_shader(DevicePtr& device, const void * blob_ptr, SIZE_T s, ID3D11ClassLinkage * l, ID3D11GeometryShaderPtr& shader_ptr)
{
  CHK(device->CreateGeometryShader(blob_ptr, s, l, &shader_ptr));
};

template <typename DevicePtr>
void create_shader(DevicePtr& device, const void * blob_ptr, SIZE_T s, ID3D11ClassLinkage * l, ID3D11PixelShaderPtr& shader_ptr)
{
...
void create_shader_from_file(Device& d3d_device, const std::wstring& path, const std::string& entry_point, const std::string& shader_model, ShaderPtr& shader, const D3D_SHADER_MACRO* defines = nullptr)
{
  ID3DBlobPtr blob;
...
fluidcs11_renderer_base::fluidcs11_renderer_base(video_renderer_resources& res, fluidcs11_renderer_base::init_params_t& p) : text_(p.title), res_(res), time_(0)
{
  ...
  // Compute Shaders
  std::wstring cspath(application::instance()->base_directory() + L"\\FluidCS11.hlsl");
  ...
  std::wstring cssortpath(application::instance()->base_directory() + L"\\ComputeShaderSort11.hlsl");
